...
class SingleInputEmbedding(eqx.Module):
    ...
    @beartype
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray: # Add type signature
...
class MultipleInputEmbedding(eqx.Module):
    ...
    def __call__(self,
                 ...
                 categorical_inputs: Optional[List[jnp.ndarray]] = None # Shape: [batch_size, in_channel]
        ...
        output = jnp.sum(jnp.stack(continuous_inputs), axis=0) 
